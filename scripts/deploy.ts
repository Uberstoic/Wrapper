...
async function main() {
  ...
  console.log(`\nDeploying with address: ${deployer.address}`);

  // Deploy Mock Token for Testing
  console.log("\nDeploying Mock Token...");
  const tokenFactory = await ethers.getContractFactory("MockERC20");
  const token = await tokenFactory.deploy("Mock Token", "MTK", 18);
  await token.deployed();
  console.log(`✅ Mock Token deployed to: ${token.address}`);

  const usdt = await ethers.getContractAt("IERC20", config.usdtMainnet);

  // Create Uniswap V3 Pool
  const poolAddress = await deployUniswapV3Pool(
  ...
  // Deploy Liquidity Wrapper
  console.log("\nDeploying Liquidity Wrapper...");
  ...
  console.log(`✅ Liquidity Wrapper deployed to: ${wrapper.address}`);
